...
\]\\	
A transfer function is defined as the output of a system with respect to a single input. All motors but one (A, in this example) are set to zero and a Laplace transform is applied to the equation:\\
...
The accuracy of the model allows for the selection of a series of test points to create a Bode diagram with enhanced precision. To perform this type of Bode analysis several frequencies will need to be chosen to create a complete diagram. Once the Bode diagram is created a new model can be backed out and controller design can be started.
